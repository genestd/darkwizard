...
const Dungeon = {
  ...
  setMinSize: function(ms){
    this.min_size = ms
  },
  setMaxSize: function(ms){
    this.max_size = ms
  },
  setMap: function(map){
    this.map = map
  ...
  connectRooms: function( roomA, roomB){
  ...
  buildWall: function(room1, room2, dir){
  ...
  findClosestRoom: function(rm, connected){
    ...
    for (var i = 0; i < 10; i++) {
      for (var j = 0; j < this.rooms.length; j++) {
        ...
        while (true) {
          ...
          if (room.x + room.w < this.map_width/2 ) room.x++;
          if (room.x + room.w > this.map_width/2 ) room.x--;
          if (room.y + room.h < this.map_height/2) room.y++;
          if (room.y + room.h > this.map_height/2) room.y--;
          if ((room.x + room.w === this.map_width/2) && (room.y + room.h === this.map_height/2)) break;
  ...
  findClosestToCenter: function(island){
    let closest = -1,
  ...
  minimize: function(){
    let map = this.getMap(),
      ...
      maxY = 0
    for( let i=0; i<rooms.length; i++){
      if (minX > rooms[i].x) minX = rooms[i].x-1
      if (maxX < rooms[i].x + rooms[i].w) maxX = rooms[i].x + rooms[i].w -1
      if (minY > rooms[i].y) minY = rooms[i].y-1
      if (maxY < rooms[i].y + rooms[i].h) maxY = rooms[i].y + rooms[i].h -1
    }
    for( let x=0; x<(maxX-minX)+2;x++){
      newMap[x]=[]
      for( let y=0; y<(maxY-minY)+2;y++){
        newMap[x][y]=map[x+minX][y+minY]
        newMap[x][y].x = x
        newMap[x][y].y = y
        newMap[x][y].type = map[x+minX][y+minY].type
      }
    }
